...
def _client():
    # import lazy: se manca la lib vediamo l’errore reale
    try:
        from amazon_paapi import AmazonApi, AmazonLocale
    ...
    return AmazonApi(
        ...
        AMAZON_SECRET_KEY,
        AMAZON_PARTNER_TAG,
        AmazonLocale.Italy,  # marketplace .it
    )
...
def search_candidates_for_keyword(keyword: str) -> List[Dict]:
    api = _client()
    # importiamo Resources QUI (non in top-level, non dentro _client)
